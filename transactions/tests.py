...
class TrasactionViewsTest(TestCase):
    ...
    def test_transaction_create_view_with_type_param(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["form"].initial["transaction_type"],
    ...
    def test_transaction_create_view(self):
        ...
        self.assertTrue(
            Transaction.objects.filter(description="Create Test Transaction").exists()
        )
    ...
    def test_transaction_update_view(self):
        ...
        response = self.client.get(
            reverse("transactions:edit", args=[self.transaction.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "transactions/transaction_form.html")
        ...
        self.assertEqual(
            self.transaction.transaction_type, TransactionType.INCOME.value
        )
